...
// { task: "Walking", completed: false }], 1)
// should give back:
//    [{ task: "Cooking", completed: true }, { task: "Walking", completed: true }]
export function toggleListItemCompleted(array, index) {
	// array of object
	// given an array and index find the object in the array
	// create a new object using the spread operator and invert the completed value (toggle the value)
	const oldObject = array[index];
	const newObject = { ...oldObject, completed: !oldObject.completed };
	return replaceItem(array, newObject, index);
}
